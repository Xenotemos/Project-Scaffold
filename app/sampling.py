...
def apply_affect_style_overrides(
    ...
    *,
    ...
    confidence = max(0.0, min(1.0, float(getattr(user_affect, "confidence", 0.0))))
    affect_weight = 0.35 + (0.65 * confidence)
    ...
    if intimacy >= 0.05 and tension <= 0.65:
        ...
        scaled_intimacy = min(1.0, intimacy * affect_weight)
        desired_tokens = int(200 + 220 * scaled_intimacy)
        desired_tokens = max(180, min(desired_tokens, 420))
        ...
        new_freq = round(max(0.05, freq_penalty - (0.18 + 0.12 * affect_weight)), 4)
        ...
        boost = (0.04 + 0.08 * scaled_intimacy) * affect_weight
        if valence >= 0.1:
            boost += 0.02 * affect_weight
        new_temp = round(min(1.32, temperature + boost), 4)
        ...
        new_top_p = round(min(0.985, top_p + 0.03 * affect_weight), 4)
        if new_top_p != top_p:
            updated["top_p"] = new_top_p
            overrides["top_p"] = new_top_p
        self_scale = float(updated.get("self_bias_scale", 0.0))
        new_self_scale = round(max(self_scale, 0.25 + 0.25 * scaled_intimacy), 4)
        if new_self_scale != self_scale:
            updated["self_bias_scale"] = new_self_scale
            overrides["self_bias_scale"] = new_self_scale
    elif (tension * affect_weight) >= 0.35 and intimacy < 0.25:
        base_tokens = int(updated.get("max_tokens", max_completion_tokens) or max_completion_tokens)
        severity = min(1.0, tension * affect_weight)
        target_tokens = max(96, min(base_tokens, int(base_tokens - (150 * severity))))
        target_tokens = min(target_tokens, 260)
        if target_tokens != base_tokens:
            updated["max_tokens"] = target_tokens
            overrides["max_tokens"] = target_tokens
        temperature = float(updated.get("temperature", base_temperature))
        new_temp = round(max(0.4, temperature - (0.05 + 0.12 * severity)), 4)
        if new_temp != temperature:
            updated["temperature"] = new_temp
            overrides["temperature"] = new_temp
        top_p = float(updated.get("top_p", base_top_p))
        new_top_p = round(max(0.45, top_p - (0.03 + 0.05 * severity)), 4)
        if new_top_p != top_p:
            updated["top_p"] = new_top_p
            overrides["top_p"] = new_top_p
    if min_tokens_floor:
